...
public class Cmd {
    ...
    public String dirRecursive(File directory){
        return bdirRecursive(directory, 0);
    }
    
    private String bdirRecursive(File directory, int level) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                result.append(String.format("%n%s%s", mezera, bdirRecursive(file, level + 1)));
            } else {
                result.append(String.format("%n%s%s", mezera,file.getName()));
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(cmd.bdirRecursive(file,0));
    }
}
